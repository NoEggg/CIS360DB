...
def insert_method(tx, row, source_file):
    ...
    """, source_file=source_file, name=row.get('name'),
...
def import_three_sheet(filepath, session, source_file):
    ...
    # Look for paper sheet
    for sheet in xls.sheet_names:
        sheet_lower = sheet.lower()
        df = pd.read_excel(filepath, sheet_name=sheet)
        df.columns = [normalize_col(col) for col in df.columns]

        if 'paper' in sheet_lower or 'publication' in sheet_lower:
            for _, row in df.iterrows():
                paper = {}
                for std_col, src_col in PAPER_COL_MAP.items():
                    if src_col in df.columns:
        ...
        elif 'dataset' in sheet_lower or 'data' in sheet_lower:
            for _, row in df.iterrows():
                dataset = {}
                for std_col, src_col in DATASET_COL_MAP.items():
                    if src_col in df.columns:
